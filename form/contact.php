...
$emailTO[] = array( 'email' => 'md.-nazrul.islam@stud.uni-bamberg.de', 'name' => 'Md. Nazrul Islam' ); 

### Enable bellow parameters & update your BCC email if require.
// $emailBCC[] = array( 'email' => 'email@youremail.com', 'name' => 'Name' );

### Enable bellow parameters & update your CC email if require.
//$emailCC[] = array( 'email' => 'email@yoursite.com', 'name' => 'Name' );

### Enter Email Subject
$subject = "Contact Us on ". $sitename; 
...
if( $_SERVER['REQUEST_METHOD'] == 'POST') {
	if (isset($_POST["contact-email"]) && $_POST["contact-email"] != '' && isset($_POST["contact-name"]) && $_POST["contact-name"] != '') {
		...
		if ($honeypot == '' && !(empty($emailTO))) {
			...
			$bodymsg .= $_SERVER['HTTP_REFERER'] ? '<br>---<br><br>Email was sent from: ' . $_SERVER['HTTP_REFERER'] : '';
